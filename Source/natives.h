...
namespace PLAYER
{
	typedef Ped(__fastcall* tGET_PLAYER_PED)(unsigned int player);
	static Ped GET_PLAYER_PED(const unsigned int& player)
	{
		static tGET_PLAYER_PED oGET_PLAYER_PED = (tGET_PLAYER_PED)(Memory::pattern("40 53 48 83 EC 20 33 DB 83 F9 FF 74 09 B2 01 E8 ? ? ? ? EB 16").count(1).get(0).get<void>(0));
		return oGET_PLAYER_PED(player);
	}
